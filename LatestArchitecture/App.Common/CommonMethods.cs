...
namespace App.Common
{
    public static class CommonMethods
    {
            ...
            try
            {
                if (authToken != null)
                {
                    ...
                    if (encryptData != null && encryptData.Claims != null)
                    {
                        ...
                        string RefreshToken = GetRefreshToken(encryptData);
                        token = new TokenModel();
                        token.UserName = Convert.ToString(encryptData.Claims[0].Value);
                        token.UserID = Convert.ToInt32(encryptData.Claims[1].Value);
                        token.StaffID = Convert.ToInt32(encryptData.Claims[2].Value);
                        token.OrganizationID = Convert.ToInt32(encryptData.Claims[3].Value);
                        token.RoleID = Convert.ToInt32(encryptData.Claims[4].Value);
                        //token.LocationID = Convert.ToInt32(encryptData.Claims[5].Value);
                        ...
                        token.DomainName = Convert.ToString(encryptData.Claims[6].Value);
                        token.Timezone = timezone;
                        token.IPAddress = ipAddress;
                        token.OffSet = Convert.ToInt32(offSet);
        ...
        public static DataTable ListToDatatable<T>(List<T> items)
        {
            ...
            PropertyInfo[] Props = typeof(T).GetProperties(BindingFlags.Public | BindingFlags.Instance).OrderBy(p => p.MetadataToken).ToArray();
        ...
        public static string CreateImageUrlSocSinature(HttpContext request, string fileName)
        {
            try
            {
                string webRootPath = Directory.GetCurrentDirectory();
                if (File.Exists(webRootPath  + fileName)) { return request.Request.Scheme + "://" + request.Request.Host + request.Request.PathBase  + fileName; }
                else { return string.Empty; }
            }
            catch (Exception)
            { return string.Empty; }
      ...
      public static string CreateHTTPRequest(string url, object data, string method, string contentType)
        ...
        {
            App.Common.JwtIssuerOptions.JwtIssuerOptions _jwtOptions = new App.Common.JwtIssuerOptions.JwtIssuerOptions();
